...
public class MqttBrokerRestartTests {
    ...
    public MqttBrokerRestartTests(ITestOutputHelper outputHelper) {
        ...
        this.factory = new TestFactory(s => s.AddMqttClient(opt => opt.ConnectionUri = new Uri("mqtt://localhost:11883")));
    }
    ...
    private async Task SendMsgAsync(string topic, int count = 1) {
        ...
        for (var i = 0; i < count; i++) {
            await pubs.PublishAsync(topic, $"{DateTime.Now}");
    ...
    public async Task TestSubscribe() {
        ...
        var sub = await subs.SubscribeAsync<string>(testTopic, MockJsonSerializerContext.Default.String);
        var reciveCount = 0;
        sub.Subscribe(e => {
            output.WriteJson(e);
            reciveCount++;
    ...
    public async Task TestChannel() {
        ...
        var channel = await subs.GetChannelAsync<string>(testTopic,MockJsonSerializerContext.Default.String);
        var reciveCount = 0;
